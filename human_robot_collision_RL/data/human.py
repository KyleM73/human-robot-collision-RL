...
class Human:
    ...
    def __init__(
        ...
        #self.constraintID = p.createConstraint(self.body_id, -1, -1, -1, p.JOINT_FIXED, [0, 0, 0], [0, 0, 0], [0, 0, 0])
    ...
    def resetGlobalTransformation(self,
                                  ...
                                  rpy=np.zeros(3),
                                  gait_phase_value=0):
        self.initial_xyz = np.array(xyz)+np.array(POSE)
        self.initial_rpy = np.array(rpy) + np.array([0, 0, -np.pi/2])
        #self.initial_rpy = np.array(rpy) + np.array([0, -np.pi/2, -np.pi/2])
    ...
    def __applyMMMRotationAndZeroTranslationToURDFBody(self, rx, ry, rz):
        ...
        #p.changeConstraint(self.constraintID, jointChildPivot=[0,0,0], jointChildFrameOrientation=r_world_to_chest, maxForce=50)
    ...
    def __applyMMMTranslationToURDFBody(self, tx, ty, tz):
        ...
        #p.changeConstraint(self.constraintID, jointChildPivot=t_final, jointChildFrameOrientation=r_final, maxForce=50)
