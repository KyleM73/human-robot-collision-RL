...
def evaluate(model_name,log_save_path=None,exp_name=EXP_NAME):
...
def evaluateOLD(model_name,log_save_path=None,exp_num=EXP_NUM):
...
def evaluateLive(model_name,exp_name=EXP_NAME):
    ...
    envTest = safetyEnv(False,rewardDict)
    modelTest = PPO.load(path, env=envTest)
